...
def get_level_posterize(level):
    return (int((level / _MAX_LEVEL) * 4),)


def get_level_solarize(level):
    return (int((level / _MAX_LEVEL) * 256),)


def get_level_solarizeadd(level):
    return (int((level / _MAX_LEVEL) * 110),)


def get_level_translate(level):
    return _translate_level_to_arg(level, 100)


def get_level_cutout(level):
    return (int((level / _MAX_LEVEL) * 40),)
...
def level_to_arg(hparams):
    return {
        "AutoContrast": dummy_fn,
        "Equalize": dummy_fn,
        "Invert": dummy_fn,
        "Rotate": _rotate_level_to_arg,
        "Posterize": get_level_posterize,
        "Solarize": get_level_solarize,
        "SolarizeAdd": get_level_solarizeadd,
        ...
        "Cutout": get_level_cutout,
        # pylint:disable=g-long-lambda
        "TranslateX": get_level_translate,
        "TranslateY": get_level_translate,
...
def randaugment(image, N, M):
    ...
    image = distort_image_with_randaugment(image, N, M)
    image = tf.image.convert_image_dtype(image, input_image_type)
    return image
